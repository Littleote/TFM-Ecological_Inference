...
cluster.model <- function(data, K, BUGS = "JAGS", iter = 2000, burn = 1000, chain = 4, thin = 10) {
  ...
  test.sim <- runBUGS(
    BUGS, c(data, test),
    parameters = c("beta", "theta.1", "omega", "zeta"), model = model.file("cluster"),
  ...
  prior <- list(
    ...
    v = weight * data$K * apply(test.matching$array[, , test.matching$at("omega")], 3, median),
  ...
  sim <- runBUGS(
    BUGS, c(data, prior),
    parameters = c("beta", "theta.1", "theta.2", "omega", "zeta"), model = model.file("cluster"),
    n.iter = iter * thin + burn, n.burnin = burn, n.thin = thin, n.chains = chain
  )
  ...
  omega <- apply(matching$array[, , matching$at("omega")], 3, median)
