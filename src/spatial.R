...
spatial.model <- function(data, ADJ, BUGS, iter = 2000, burn = 1000, chain = 4, thin = 10) {
  ...
  sim <- runBUGS(
    ...
    data = c(data, prior),
    parameters = c(
      "beta", "theta.2", "delta", "nu", "tau.v",
      (if (null.effect) NULL else c("upsilon", "tau.u"))
    ),
    model = model.file(glue("spatial{ifelse(null.effect, '-null_effect', '')}")),
  ...
  return(c(
    ...
    (if (!null.effect) list(upsilon = get.array(apply(arr[, , at("upsilon")], 3, median), dim(ref$upsilon))) else NULL)
  ))
}
